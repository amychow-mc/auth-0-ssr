function verify(email, callback) {
  ...
  const checkUserUrl =
    "https://c161-202-64-44-178.ngrok-free.app/api/check-user-by-credentials"; // API to check user
  ...
  request.post(
    ...
    (checkError, checkResponse, checkBody) => {
      if (checkError) {
        console.error(
          `[changePassword] Error checking user: ${checkError.message}`
        );
      ...
      if (checkResponse.statusCode === 404) {
        console.log(
          `[changePassword] User not found. Creating a new user in Auth0.`
        );
        ...
        if (!user.isMigrated) {
          console.log(
            `[changePassword] User is not migrated. Creating the user in Auth0.`
          );
          // createUserInAuth0(email, newPassword);
          createUserInAuth0customdb(email, newPassword);
          return callback(null, true);
          ...
          console.log(
            `[changePassword] User is migrated. Sending password reset email.`
          );
        ...
        console.error(
          `[changePassword] Unexpected response: ${checkResponse.statusCode}`
        );
        return callback(
          new Error("Unexpected response while checking user existence.")
        );
      }
    }
  );
  ...
  function createUserInAuth0(email, password) {
    ...
    request.post(
      createUserOptions,
      (createError, createResponse, createBody) => {
        if (createError) {
          console.error(
            `[AUTH0 CREATE USER MAIN] Error: ${createError.message}`
          );
        ...
        if (createResponse.statusCode !== 201) {
          console.error(
            `[AUTH0 CREATE USER MAIN] Failed: ${createResponse.statusCode} - ${createBody.message}`
          );
          // return callback(new Error("Failed to create user in Auth0."));
        }

        console.log(
          `[AUTH0 CREATE USER MAIN] User successfully created in Auth0: ${JSON.stringify(
            createBody
          )}`
        );
  ...
  function createUserInAuth0customdb(email, password) {
    ...
    request.post(
      ...
      (createError, createResponse, createBody) => {
        if (createError) {
          console.error(
            `[AUTH0 CREATE USER CUSTOM] Error: ${createError.message}`
          );
          // return callback(new Error("Error creating user in Auth0."));
        }
        ...
        if (createResponse.statusCode !== 201) {
          console.error(
            `[AUTH0 CREATE USER CUSTOM] Failed: ${createResponse.statusCode} - ${createBody.message}`
          );
          // return callback(new Error("Failed to create user in Auth0."));
        }

        console.log(
          `[AUTH0 CREATE USER CUSTOM] User successfully created in Auth0: ${JSON.stringify(
            createBody
          )}`
        );
        // callback(null, true); // User creation successful
  ...
  function sendPasswordResetEmail(email) {
    ...
    request.post(
      ...
      (resetError, resetResponse, resetBody) => {
        if (resetError) {
          console.error(
            `[AUTH0 PASSWORD RESET EMAIL] Error: ${resetError.message}`
          );
          // return callback(new Error("Error sending password reset email."));
        } else if (resetResponse.statusCode !== 200) {
          console.error(
            `[AUTH0 PASSWORD RESET EMAIL] Failed: ${resetResponse.statusCode} - ${resetBody.message}`
          );
          // return callback(new Error("Failed to send password reset email."));
        } else {
          console.log(
            `[AUTH0 PASSWORD RESET EMAIL] Password reset email sent successfully.`
          );
