function changePassword(email, newPassword, callback) {
    ...
    request.get(getUserOptions, function (getUserError, getUserResponse, getUserBody) {
        if (getUserError) {
            console.error(`[changePassword] Error fetching user: ${getUserError.message}`);
            return callback(new Error("Error fetching user."));
        }

        if (!getUserBody || getUserBody.length === 0) {
            console.error(`[changePassword] No user found with email: ${email}`);
            return callback(new Error("User not found."));
        }

        const user = getUserBody.find(user => 
            user.identities.some(identity => identity.connection === "Username-Password-Authentication")
        );

        if (!user) {
            console.error(`[changePassword] No user found with connection: Username-Password-Authentication`);
        ...
        request.patch(options, function(err, response, body) {
            ...
            console.log(`[AUTH0 MAIN UPDATE PASSWORD] Response: ${JSON.stringify(body)}`);
